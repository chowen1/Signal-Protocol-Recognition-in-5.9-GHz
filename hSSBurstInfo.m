...
function info = hSSBurstInfo(burst)
    ...
    % achieve the appropriate sample rate. The largest number of resource
    % blocks yielding the appropriate sample rate is selected, in order
    % to maximize the flexibility of SS burst placement. If the required
    % number of resource blocks exceeds 275, then 275 resource blocks are
    % used within a larger FFT size
    nfft = burst.SampleRate / (burst.SubcarrierSpacing * 1e3);
    burst.NRB = min(floor(nfft * 0.85 / 12),275);
    if (burst.NRB < 20)
    ...
    ofdmInfo = nrOFDMInfo(burst.NRB,burst.SubcarrierSpacing,'CyclicPrefix',burst.CyclicPrefix,'Windowing',burst.Windowing,'Nfft',nfft);
    ...
    if ((burst.FrequencyPointA < -gridFreqSpan) || (burst.FrequencyPointA > (gridFreqSpan - (burst.SubcarrierSpacingCommon * 1e3))))
        error('Resource grid (NRB = %d, spanning %0.3f .... %0.3f MHz) configured by sample rate (%0.2f MHz) and SSB subcarrier spacing (%d kHz) does not span frequency point A (%0.3f MHz).',burst.NRB,-gridFreqSpan/1e6,(gridFreqSpan - (burst.SubcarrierSpacingCommon*1e3))/1e6,burst.SampleRate/1e6,burst.SubcarrierSpacing,burst.FrequencyPointA/1e6);
    end
    ...
    nSubcarriers = burst.NRB * 12;
    k0_offset = nSubcarriers/2 + (ssbf0 / (burst.SubcarrierSpacing * 1e3));
    if ((k0_offset < 0) || (k0_offset > (nSubcarriers - 240)))
