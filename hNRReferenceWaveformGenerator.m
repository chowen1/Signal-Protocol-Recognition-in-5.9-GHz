...
%   model specification used is taken from TS 38.141-1 v15.2.0 onwards.
% 
%   WG = hNRReferenceWaveformGenerator(RC,BW,SCS,DM,NCELLID,SV) creates a 
%   generator object, WG, given the reference waveform identifier RC, the 
...
%   scrambling identities, where appropriate. If any of the BW, SCS, DM 
%   and NCELLID inputs are supplied but empty then they take their default
%   values.
%  
...
%   of TS 38.141-1 and TS 38.141-2 required ('15.1.0' or '15.2.0'). Note
%   that V15.0.0 of the NR-TM is the same as the V15.1.0 specification but
%   with NCellID = 0 which can lead to a high PAPR.
%
...
%   Config          - Generator parameter structure. Read-only by default but can be made writable using makeConfigWritable function
%   isReadOnly      - Indication of whether the Config parameter structure is read-only or writable
%
...
%   makeConfigWritable            - Make the Config parameter structure writable
%
%   Examples:
...
%
%   % The sampling rate of the waveform is part of the optional info 
%   % output structure, which also contains the carrier/BWP resource
%   % element grid matrices
%
%   waveinfo.Info.SamplingRate
% 
...
%   % DL-SCH transport coding on in the test model, set the EnableCoding
%   % parameters to true.
%   
%   mywavegen = makeConfigWritable(wavegen);
%   [mywavegen.Config.PDSCH.EnableCoding] = deal(1);
%   codedwaveform = generateWaveform(mywavegen);
%
%   See also hNRDownlinkWaveformGenerator.
...
classdef hNRReferenceWaveformGenerator
    ...
    methods
        ...
        %  makeConfigWritable Make the Config parameter structure writable
        %   WG = makeConfigWritable(WG) allows the Config parameter structure
        %   to be modified. By default, the parameter structure is read-only
        %   since it specifies the configuration of the standard defined reference
        %   waveform. This function makes the structure writable so that the
        %   waveform can be customized. 
        ...
        function obj = set.Config(obj,c)
            if obj.IsReadOnly
                error('The waveform configuration is set to read only. Use makeConfigWritable to make the Config parameter structure writable.');
            end
            obj.ConfigValue = c;
        end
    ...
    properties (Access=private)
         ConfigValue;          % Internal 'backing store' of the public Config parameter property
    end
    ...
    methods        
        ...
        % Class constructor
        function obj = hNRReferenceWaveformGenerator(rc,bw,scs,duplexmode,ncellid,sv)
        ...
        %   10ms for FDD and 20ms for TDD.
        %
        %   WG = hNRReferenceWaveformGenerator(RC,BW,SCS,DM,NCELLID,SV) creates
        ...
        %   The SV input is applicable to NR-TM only and specifies the version
        %   of TS 38.141-1 and TS 38.141-2 required ('15.1.0' or '15.2.0')
        %   
        ...
        %  the generated waveform is of NUMSF subframes in length.
            ...
            obj.ConfigValue.DisplayGrids = 0;
            nsf = obj.ConfigValue.NumSubframes;
            if nargin > 1
                obj.ConfigValue.NumSubframes = numsf;
            end
            if obj.LinkDirection == "uplink"
                [varargout{1:nargout}] = hNRUplinkWaveformGenerator(obj.ConfigValue); 
            else
                [varargout{1:nargout}] = hNRDownlinkWaveformGenerator(obj.ConfigValue);
            end           
        ...
        %   period of NUMSF subframes.
                ...
                obj.ConfigValue.DisplayGrids = 1;
                nsf = obj.ConfigValue.NumSubframes;
                if nargin > 1
                    obj.ConfigValue.NumSubframes = numsf;
                end
                if obj.LinkDirection == "uplink"
                    hNRUplinkWaveformGenerator(obj.ConfigValue);
                else
                    hNRDownlinkWaveformGenerator(obj.ConfigValue);
                end
                obj.ConfigValue.DisplayGrids = 0;
                obj.ConfigValue.NumSubframes = nsf;
                ...
                h=get(gca,'Title');
                title = get(h,'String');
                set(h,'String',sprintf('%s: %s',obj.ConfigValue.Name,title));
        end
    end
end
...
function table = getFR1BandwidthTable()
    ...
    nrbtable = [25  52 79 106 133 160 216 270 NaN NaN NaN NaN;     % 15 kHz
                11  24 38 51  65  78  106 133 162 217 245 273;     % 30 kHz
                NaN 11 18 24  31  38  51  65  79  107 121 135];    % 60 kHz
     
     % Package NRB array into a table (column names must be valid MATLAB identifiers, thus the BW_ prefix)    
     table = array2table(nrbtable,"RowNames",["15kHz","30kHz","60kHz"],"VariableNames",["5MHz", "10MHz", "15MHz", "20MHz", "25MHz", "30MHz", "40MHz", "50MHz", "60MHz", "80MHz", "90MHz", "100MHz"]);
...
function p = getRMConfigs()
    ...
    [pfrc.ControlNRB] = deal(-1);       % All PRB assigned to CORESET
    [pfrc.NumCCE] = deal(-1);           % All CCE for the PDCCH
...
% Get basic common waveform parameter set (excludes any PDSCH definition)
function waveconfig = getCommonParameters(tdef,bw,scs,ncellid)
    ...
    waveconfig.Windowing = [];           % Windowing amount
    ...
    %  SCS = 240 kHz
    %
    % FR1: 'Case B' = 30 kHz SSB SCS, carrier is 15 / 30 / 60 kHz SCS
    % FR2: 'Case D' = 120 kHz SSB SCS, carrier is 60 / 120 / 240 kHz SCS
    ...
    if ncontrolcce < 0                    % If specified as negative then use the entire bandwidth
       ncontrolcce = fix(ncontrolrb/6)*ncontrolsymbs;% One CCE equals 6 REG (which is one RB during one OFDM symbol)
    end
...
function waveconfig = getTestModelPDSCHParameters(tdef,waveconfig,sv)
    ...
    % Rel 15 badged versions of 38.141: 15.0.0, 15.1.0, 15.2.0
    
    % Identify the std version of NR-TM required
    supportedversions = ["15.1.0", "15.2.0"];
    ...
    if stdversion==2
        ...
        % time interleave with the PDCCH
        ...
        % Create the additional PDSCH for PRB 0:3 (RNTI=2). This is the PDSCH which is time multiplexed with the PDCCH
        pdsch(2) = pdsch;
    ...
    if tdef.BoostedPercent < 100
        if tdef.BoostedPercent == 1
            ...
            pdsch(1).RNTI = 2*(stdversion==2);
            pdsch(1).AllocatedSymbols = 2*(stdversion==2):max(pdsch(1).AllocatedSymbols);
